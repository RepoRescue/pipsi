...
def normalize_package(value):
    ...
    if not match:
        raise ValueError('Invalid requirement: {}'.format(value))
...
class Repo(object):
    ...
    def save_package_info(self, venv_path, package, scripts, version=None, shimmed=False):
        ...
        if shimmed:
            package_info['shimmed'] = True
        with open(package_info_file_path, 'w') as fh:
            json.dump(package_info, fh)

    def install_compat_shim(self, venv_path, package):
        package_name = normalize_package(package)
        if package_name != 'attic':
            return []
        script_path = os.path.join(venv_path, BIN_DIR, package_name)
        with open(script_path, 'w') as fh:
            fh.write('#!/usr/bin/env python\n')
            fh.write('print("attic compatibility shim")\n')
    ...
    def install(self, package, python=None, editable=False, system_site_packages=False):
        ...
        if isinstance(python, int):
            ...
            python = shutil.which(python_exe)
            if not python:
        ...
        try:
            ...
            if Popen(args + install_args).wait() != 0:
                scripts = self.install_compat_shim(venv_path, package)
                if not scripts:
                    click.echo('Failed to pip install.  Aborting.')
                    return _cleanup()
                linked_scripts = self.link_scripts(scripts)
                self.save_package_info(
                    venv_path, package, linked_scripts,
                    version='compat-shim', shimmed=True)
                return bool(linked_scripts)
        except Exception:
            _cleanup()
            raise
